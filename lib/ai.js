...
export const getGeminiModel = async (modelName = "gemini-2.5-flash") => {
  return genAI.getGenerativeModel({ model: modelName });
};

/**
 * Safely parse JSON from Gemini AI response
 * Handles markdown code blocks and invalid JSON
 */
export const parseAIJsonResponse = async (text) => {
...
export const generateJsonResponse = async (prompt, maxRetries = 3) => {
  ...
  return await parseAIJsonResponse(result.text);
};
